...
{
    public function buscarPorId(int $id): ?Escuela { $r = $this->one('SELECT * FROM escuela WHERE id_escuela = :id', [':id' => $id]); return $r ? $this->map($r) : null; }
    public function listar(): array { return array_map(fn(array $r): Escuela => $this->map($r), $this->all('SELECT * FROM escuela ORDER BY nombre')); }

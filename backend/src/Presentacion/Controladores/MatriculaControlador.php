...
final class MatriculaControlador
{
    ...
    public function registrar(array $datos): array
    {
        try {
            ...
            if (!empty($datos['codigo_matricula'])) {
                $matricula->setCodigoMatricula((string)$datos['codigo_matricula']);
            }
    ...
    {
        $codigo = $m->getCodigoMatricula();
        $codigoStr = is_object($codigo) && method_exists($codigo, 'getValue')
            ? $codigo->getValue()
